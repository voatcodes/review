...
app.post('/business/edit-profile/:id', upload.single('photoURL'), (req, res) => {
    const profile = {
        business: {
            name: req.body.businessName,
            email: req.body.businessEmail,
    ...
    bcrypt.compare(profile.rep.password, res.locals.business.b_password, (error, matches) => {
        if (matches) {
            ...
            if (req.file) {
                profile.business.photoURL = req.file.filename

                let sql = 'UPDATE  business_profile SET b_name = ?, b_email = ?, b_tag_line = ?, b_category = ?, b_description = ?, b_location = ?, b_photoURL = ?, b_contact_person = ?, b_phone_number = ?, b_email_address = ? WHERE b_id = ?'
                connection.query(
                    sql,
                    [
                        profile.business.name,
